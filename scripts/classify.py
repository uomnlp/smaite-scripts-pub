...
class DataTrainingArguments:
    ...
    label_column: str = field(metadata={"help": "Name of label column."})
    input_column: List[str] = field(
        metadata={"help": "Name of input columns."})
    is_regression: Optional[int] = field(
        default=False, metadata={"help": "If task is regression."})
...
def main():
    ...
    if is_regression:
        ...
        label_list = [l for l in label_list if l is not None]
    ...
    def preprocess_function(examples):
        # Tokenize the texts
        texts, labels = [], []
        for values in zip(*(examples[field] for field in ([data_args.label_column] + data_args.input_column))):
            label, *inputs = values
            text = '\n'.join(inputs)
            if text and label != None and label != 'None' and label != 0:
                texts.append(text)
                labels.append(label)
        # examples = [e for e in examples if any(e[field] for field in data_args.input_column)
        #             and e[data_args.label_column] is not None and e[data_args.label_column] != 0]
        # texts = ['\n'.join(example[field] for field in data_args.input_column)
        ...
        if data_args.is_regression:
            result["label"] = [(l-1)/4 for l in labels]
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        if data_args.is_regression:
            results = {
                'mse': sklearn.metrics.mean_squared_error(preds, p.label_ids)
            }
        else:
            results = {
